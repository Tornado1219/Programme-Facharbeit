...
void Kalibieren() { //Funktion zur Kalibrierung des LDR und finden vom Umgebungsabhängigen Schwellenwert
...
void loop() {
  //Warten auf Serielle Eingaben
  if (Serial.available()) {
    String txt = Serial.readString();
    Serial.println(txt);

    if (txt.startsWith("start")) { //Start einer Messsequenz ohne EMS
      ...
      Serial.println(t);
      digitalWrite(LEDpin, LOW);
    } else if (txt.startsWith("EMSstart")) { //Start einer Messsequenz mit EMS
      if (FotoSchwelle == 0) {
        Kalibieren();
      }
      Serial.println("Random delay gestartet");
      ...
      Kalibieren();
    } else if (txt.startsWith("EMSon")) { //dierekte Ansteuerung EMS zum finden der EMS Ansatzpunkte
      digitalWrite(Relaispin, HIGH);
